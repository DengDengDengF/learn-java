...
> 每秒 100 笔支付请求，每笔请求平均耗时 100ms，10 个线程刚好能扛住（100×0.1=10）

1 / 0.1 = 10 ,分成10个区间

100 / 10 = 10,这一个区间内部有10个并发
...
| 隔离级别                     | 核心思想                                 | 脏读 Dirty Read | 不可重复读 Non-Repeatable Read | 幻读 Phantom Read               | 具体例子                                                     | 优点                 | 缺点                           |
| ---------------------------- | ---------------------------------------- | --------------- | ------------------------------ | ------------------------------- | ------------------------------------------------------------ | -------------------- | ------------------------------ |
| **Read Uncommitted读未提交** | 不管别人事务有没有提交，都可以读取       | ✅ 会发生        | ✅ 会发生                       | ✅ 会发生                        | **A修改余额100→50，但没提交；B查询看到50；A回滚，实际还是100** | 性能最高，并发最高   | 数据可能是假数据，几乎不用     |
| **Read Committed读已提交**   | 只能读取已经提交的数据                   | ❌ 不会发生      | ✅ 会发生                       | ✅ 会发生                        | **A第一次查询余额100；B修改余额50并提交；A第二次查询变50**   | 避免脏读，一致性较好 | 同一个事务中，同一数据可能变化 |
| **Repeatable Read可重复读**  | 一个事务开始后，多次读取同一数据保持一致 | ❌ 不会发生      | ❌ 不会发生                     | ⚠️ 标准SQL会发生MySQL InnoDB解决 | **A查询年龄20的人有3个；B插入一个年龄20的人提交；A再次查询仍看到原快照（MySQL）** | 一致性强，性能好     | 锁和MVCC实现更复杂             |
| **Serializable串行化**       | 事务完全串行执行，一个执行完另一个再执行 | ❌ 不会发生      | ❌ 不会发生                     | ❌ 不会发生                      | **A查询年龄20的人，数据库锁住范围；B不能插入年龄20的数据，必须等待A完成** | 数据绝对安全         | 并发最低，性能差               |

解释:

| 问题           | 关注点                     | 例子                                          |
| -------------- | -------------------------- | --------------------------------------------- |
| **脏读**       | 读到了别人**没提交的数据** | A改100→50，没提交，B读50，A回滚               |
| **不可重复读** | 同一行数据，**值变了**     | A第一次读100，B改50提交，A第二次读50          |
| **幻读**       | 同一个查询，**行数变了**   | A查20岁用户3个，B插入一个20岁用户，A再次查4个 |
...
UPDATE/DELETE：
    操作最新版本
----------------------
数据库保证“数据不会乱”，业务保证“业务规则不会错”。
RR 解决的是前者，后者通常需要业务设计。
业务数据如下:
A:1000 扣 800 = 200,version = 1 
B:1000 扣 500 = 500,version = 1
A先成功,version = 2
B版本不对,重试
